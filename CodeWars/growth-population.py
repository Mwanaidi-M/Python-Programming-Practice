...
p0, percent, aug (inhabitants coming or leaving each year), p (population to equal or surpass) the
...
def nb_year(p0, percent, aug, p):
    # initialize a variable to count number of years passed
    count = 0

    # while the current population is less than the target population
    while p0 < p:
        # calculate the total population in a year
        total_pop = int(p0 + p0 * (percent / 100) + aug)

        # set the total population as the current population
        p0 = total_pop
